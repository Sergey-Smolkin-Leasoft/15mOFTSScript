...
from config import (
    ...
    DAILY_CHECK_TIME_UTC, LOOKBACK_WINDOW_FOR_SCAN, # Import new parameters
    TIMEFRAME_DURATIONS_MINUTES # Import for calculations
)
...
def calculate_position_size(account_balance, risk_percent, risk_fixed, stop_loss_pips, asset, assumed_pip_value_per_lot):
    ...
    risk_percent: процент риска на сделку (например, 1.0 для 1%) или None
    risk_fixed: фиксированная сумма риска в валюте счета или None
    stop_loss_pips: размер стоп-лосса в пипсах
    asset: символ актива
    assumed_pip_value_per_lot: предполагаемая стоимость пипса для 1 стандартного лота
    """
...
def calculate_pips(price1, price2, asset):
     ...
     if price1 is None or price2 is None:
         return 0.0
     # Определяем размер пипса: 0.0001 для большинства пар, 0.01 для JPY пар
     pip_decimal_places = 4
     if "JPY" in asset:
         pip_decimal_places = 2

     # Пипс фактор: 0.0001 или 0.01
...
class Backtester:
    ...
    def run(self):
        ...
        fetch_start_date = self.start_date - self.lookback_window_for_scan * 2 # Добавим запас
        fetch_end_date = self.end_date + self.m15_timedelta # Загрузим немного данных после конца на случай закрытия последней свечой
        ...
        if trading_days_in_data.empty:
             print("Нет загруженных данных M15. Бэктест отменен.")
             return

        # Фильтруем дни, чтобы начать с дня self.start_date и закончить self.end_date
        # trading_days = trading_days_in_data[(trading_days_in_data >= self.start_date.normalize()) & (trading_days_in_data <= self.end_date.normalize())]
        # Упростим: итерируем по всем дням в загруженных данных, но проверяем check_time
        ...
        last_check_time = actual_data_start # Для первой проверки SL/TP
        ...
        for trading_day in trading_days:
            ...
            # И убедимся, что есть достаточно данных для lookback перед check_time
            ...
            if current_check_time < self.start_date or current_check_time > self.end_date or required_data_start_for_scan < actual_data_start:
                # print(f"\nВремя проверки {current_check_time} вне диапазона бэктеста или недостаточно данных для lookback. Пропускаем день.")
                last_check_time = current_check_time # Обновляем last_check_time даже если пропускаем день, чтобы не проверять SL/TP по всему прошлому
                continue


            # print(f"\nОбработка дня и проверка в {current_check_time}") # Отладочный вывод

            # --- Обработка открытой позиции (проверка SL/TP с last_check_time до current_check_time) ---
            if self.open_position:
                pos = self.open_position
                check_sl_tp_start_time = last_check_time # Начинаем проверку сразу после предыдущего check_time

                # Срез данных M15 для проверки SL/TP: от check_sl_tp_start_time до current_check_time (включительно для последней свечи)
                ...
                if not data_for_sl_tp_check.empty:
                    for candle_time, candle_data in data_for_sl_tp_check.iterrows():
                        ...
                        if hit_sl or hit_tp:
                            ...
                            break # Выходим из цикла проверки SL/TP для этой позиции
            ...
            if self.open_position is None:
                ...
                data_m15_scan_window = data_m15_full.loc[(data_m15_full.index > current_check_time - self.lookback_window_for_scan) & (data_m15_full.index <= current_check_time)].copy()
                data_h1_scan_window = data_h1_full.loc[(data_h1_full.index > current_check_time - self.lookback_window_for_scan) & (data_h1_full.index <= current_check_time)].copy()

                min_candles_needed_m15 = LIQUIDITY_LOOKBACK * 2 + 5
                h1_duration_min = TIMEFRAME_DURATIONS_MINUTES.get('H1', 60)
                m15_duration_min = TIMEFRAME_DURATIONS_MINUTES.get('M15', 15)
                min_h1_candles_needed = int(min_candles_needed_m15 * (m15_duration_min / h1_duration_min))
                min_h1_candles_needed = max(1, min_h1_candles_needed)


                if len(data_m15_scan_window) < min_candles_needed_m15 or len(data_h1_scan_window) < min_h1_candles_needed:
                    # print(f"  Недостаточно данных в окне сканирования до {current_check_time} для стратегии (M15: {len(data_m15_scan_window)}, H1: {len(data_h1_scan_window)}). Пропускаем сигнал для этого дня.")
                    pass # Не генерируем сигнал, но продолжаем обработку дня

                else:
                    # print(f"  Запуск generate_signal на основе данных до {current_check_time}") # Отладочный лог
                    ...
                    # Если сигнал сгенерирован, имитируем открытие позиции
                    if signal != 'NONE' and entry is not None and sl is not None and tp is not None:
                        # Имитация исполнения ордера:
                        # Найдем первую свечу M15 ПОСЛЕ current_check_time для имитации входа
                        ...
                        if pd.isna(entry_candle_time) or entry_candle_time > self.end_date:
                             ...
                             pass # Нет свечей для входа после check_time в пределах бэктеста

                        else:
                            # Имитируем вход по цене открытия следующей свечи
                            entry_price_actual = data_m15_full.loc[entry_candle_time]['Open']

                            # Пересчитываем SL/TP относительно фактической цены входа, сохраняя размер стопа в пипсах
                            ...
                            if calculated_stop_loss_pips is None or calculated_stop_loss_pips <= 0:
                                ...
                                # Новые уровни SL/TP от фактической цены входа
                                ...
                                if signal == 'BUY':
                                    ...
                                    # Сохраняем расстояние до TP в пипсах или цене
                                    # Расстояние TP от *расчетного* входа в пипсах
                                ...
                                # Проверяем, что фактические SL/TP находятся на правильной стороне от фактической цены входа
                                if (signal == 'BUY' and (sl_actual >= entry_price_actual or tp_actual <= entry_price_actual)) or \
                                   (signal == 'SELL' and (sl_actual <= entry_price_actual or tp_actual >= entry_price_actual)):
                                    ...
                                    # Пересчитываем RR относительно фактической цены входа и фактических SL/TP
                                    ...
                                    if rr_actual is None or rr_actual < MIN_RR:
                                        ...
                                        if volume_lots > 0:
                                            self.open_position = {
                                                ...
                                                'last_check_time': current_check_time # Сохраняем время проверки
                                            }
                                        ...
                                        # else:
                                            # print(f"  Сигнал {signal}, но нулевой объем лотов рассчитан. Вход не выполнен.")


            last_check_time = current_check_time # Обновляем время последней проверки

            # Обновляем максимальную просадку после каждого дня
            if self.current_balance > self.peak_balance:
                 self.peak_balance = self.current_balance
            drawdown = self.peak_balance - self.current_balance
            if drawdown > self.max_drawdown:
        ...
        # --- Проверка закрытия открытой позиции с момента последней проверки до конца данных ---
        if self.open_position:
             pos = self.open_position
             # Проверяем свечи M15 с момента last_check_time до фактического конца данных
             ...
             if not data_for_sl_tp_check.empty:
                 for candle_time, candle_data in data_for_sl_tp_check.iterrows():
                     ...
                     if hit_sl or hit_tp:
                         ...
                         self.open_position = None
                         print(f"  Позиция закрыта ({trade_result['exit_reason']}) на {trade_result['exit_time']}. PnL: {trade_result['pnl_account_currency']:.2f}. Баланс: {self.current_balance:.2f}")
                         break # Выходим из цикла проверки SL/TP


        # Если бэктест закончился, а позиция все еще открыта, закрываем ее по цене закрытия последней свечи
        if self.open_position:
             pos = self.open_position
...
if __name__ == "__main__":
    ...
    # Проверка, что задан хотя бы один тип риска
    ...
    # Проверка наличия TIMEFRAME_DURATIONS_MINUTES и корректных значений
